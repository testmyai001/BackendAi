...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  // Count of invoices explicitly in "Ready" state waiting for push
  const readyToPushFiles = files.filter(f => f.status === 'Ready').length;
    ...
    <div className="flex flex-col h-full w-full overflow-auto">
      <div ref={scrollContainerRef} className="flex flex-col gap-6 pb-4 px-6 animate-fade-in relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 flex-shrink-0">
          ...
          <span className="text-sm font-medium text-slate-500 dark:text-slate-400">Total Invoices<br/>Uploaded</span>
          </div>
          ...
          <span className="text-sm font-medium text-emerald-800 dark:text-emerald-400">Successful<br/>Invoices</span>
          </div>
          ...
          <span className="text-sm font-medium text-red-800 dark:text-red-400">Failed<br/>Invoices</span>
          </div>
          ...
          <span className="text-sm font-medium text-blue-800 dark:text-blue-400">Correct Entries<br/>Passed</span>
          </div>
             ...
             <div className="relative">
                ...
                {isFilterOpen && (
                   <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl z-50 overflow-hidden animate-fade-in">
                      {['All', 'Success', 'Ready', 'Failed', 'Processing', 'Sales', 'Purchase'].map(status => (
                          <button
                            key={status}
           ...
           <table className="w-full text-sm text-left">
            <thead className="text-xs text-slate-500 dark:text-slate-400 uppercase bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold text-center whitespace-nowrap">View File</th>
                <th className="px-6 py-4 font-semibold whitespace-nowrap">Status</th>
                <th className="px-6 py-4 font-semibold text-center whitespace-nowrap">Correct Entries</th>
                <th className="px-6 py-4 font-semibold text-center whitespace-nowrap">Incorrect Entries</th>
                <th className="px-6 py-4 font-semibold whitespace-nowrap">Reason for Failure</th>
                  ...
                  <tr key={file.id} className="bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        `}>
                            ...
                            {file.status === 'Success' ? 'Pushed' : file.status}
                        </span>
                    </td>
                    <td className="px-6 py-4 text-center font-mono font-medium text-slate-700 dark:text-slate-300">{file.correctEntries.toString().padStart(2, '0')}</td>
                    <td className="px-6 py-4 text-center font-mono font-medium text-slate-700 dark:text-slate-300">{file.incorrectEntries.toString().padStart(2, '0')}</td>
                    <td className="px-6 py-4 text-slate-500 text-xs max-w-[200px] truncate" title={file.error || '-'}>
                        {file.error || '-'}
                    </td>
        ...
        <div className="p-6 border-t border-slate-200 dark:border-slate-700">
           ...
           <div className="flex flex-col md:flex-row justify-between items-center gap-6">
              <div className="flex flex-wrap gap-x-8 gap-y-2 text-sm">
                 <span className="text-slate-600 dark:text-slate-400">Total Invoices Processed: <strong className="text-slate-900 dark:text-white ml-1">{totalFiles}</strong></span>
                 <span className="text-slate-600 dark:text-slate-400">Correct Entries: <strong className="text-emerald-600 dark:text-emerald-400 ml-1">{totalCorrectEntries}</strong></span>
                 <span className="text-slate-600 dark:text-slate-400">Incorrect Entries: <strong className="text-orange-600 dark:text-orange-400 ml-1">{totalIncorrectEntries}</strong></span>
              </div>
