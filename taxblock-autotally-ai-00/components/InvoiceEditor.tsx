...
const InvoiceEditor: React.FC<InvoiceEditorProps> = ({ 
  ...
  const validationErrors = useMemo(() => {
    ...
    formData.lineItems.forEach(item => {
         ...
         // Allow small tolerance (e.g. 1.0) for rounding differences in source doc
  ...
  // Calculate totals whenever line items or voucher type changes
  useEffect(() => {
    let totalTaxable = 0;
    let totalTax = 0;
    let totalCGST = 0;
    ...
    // Determine Place of Supply (Basic Logic)
    ...
    // Assume Local if one is missing (simplification for user edits) OR if state codes match
    const isInterState = supplierState && buyerState && supplierState !== buyerState;

    formData.lineItems.forEach(item => {
      // Use item.amount directly (assuming it is rounded during input change)
      const amount = Number(item.amount) || 0;
      totalTaxable += amount;

      // Calculate tax on rounded amount per line (Strict Tally logic)
      ...
      if (gst > 0) {
          if (isInterState) {
              ...
              // Local: Split Tax 50/50
              const half = round(lineTax / 2);
              const otherHalf = round(lineTax - half); // handle odd numbers/rounding
              totalCGST += half;
              totalSGST += otherHalf;
          }
      }
  ...
  }, [formData.lineItems, formData.supplierGstin, formData.buyerGstin]);
  ...
  const handleLineItemChange = (id: string, field: keyof LineItem, value: string | number) => {
    setFormData(prev => ({
      ...
      lineItems: prev.lineItems.map(item => {
        ...
        // Auto-calculate amount if qty or rate changes using STRICT ROUNDING
  ...
  const handleSave = () => {
      onSave(formData, false); // False = Stay on editor tab
  };
        ...
        <div className="flex-1 overflow-auto p-6 scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-600 scrollbar-track-transparent">
            ...
            <div className="space-y-4">
                ...
                <p className="text-[10px] text-slate-400 mt-1">
                    {companies.length === 0 && !loadingCompanies ? "No companies found or CORS blocked." : `${companies.length} companies available`}
                </p>
                </div>
                ...
                />
                <p className="text-[10px] text-slate-400 mt-1">Format: DD-MM-YYYY</p>
                </div>
            </div>
            ...
            <div className="overflow-x-auto border border-slate-200 dark:border-slate-700 rounded-lg">
                ...
                <thead className="bg-slate-50 dark:bg-slate-700/50 text-slate-600 dark:text-slate-300 font-semibold border-b border-slate-200 dark:border-slate-700 sticky top-0 z-0">
                    ...
                    <th className="px-4 py-3 w-24 bg-slate-50 dark:bg-slate-800">Unit</th>
                    ...
                    <th className="px-4 py-3 w-24 bg-slate-50 dark:bg-slate-800">GST %</th>
                    ...
                    <tr key={item.id} className="group hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                        ...
                        <td className="p-2 font-mono font-medium">
                            <input 
                                ...
                                className={getTableInputClass(hasError('lineItemMath', item.id))}
                                title={hasError('lineItemMath', item.id) ? "Amount does not match Qty * Rate" : ""}
                            />
                        </td>
                        <td className="p-2">
            ...
            <div className="w-72 space-y-3 bg-slate-50 dark:bg-slate-900/50 p-4 rounded-lg border border-slate-200 dark:border-slate-700">
