...
import { UploadCloud, FileText, ArrowRight, Loader2, CheckCircle2, AlertTriangle, Trash2, Landmark, Save, History } from 'lucide-react';
...
const BankStatementManager: React.FC<BankStatementManagerProps> = ({ 
  ...
  const [step, setStep] = useState<1 | 2>(1); // 1: Upload, 2: Review & Push
  ...
  const processFile = async (uploadedFile: File) => {
      ...
      // Register with Dashboard if handler provided
      ...
      setShowInvoiceAlert(false); // Reset alert
      ...
      try {
        ...
        if (result.documentType === 'INVOICE') {
            ...
            if (onUpdateFile && currentFileId) {
                onUpdateFile(currentFileId, { status: 'Mismatch', error: 'Detected as Invoice' });
            }
            return;
        }
        ...
        if (onUpdateFile && currentFileId) {
            onUpdateFile(currentFileId, { status: 'Failed', error: 'Parsing Failed' });
        }
      } finally {
        setIsProcessing(false);
      }
  ...
  // Helper to guess ledger based on description
  const guessLedgerFromDescription = (desc: string): string => {
    ...
    // Payment Modes (Generic Fallbacks)
  ...
  const handleTransactionChange = (id: string, field: keyof BankTransaction, value: string | number) => {
    setData(prev => ({
      ...
      transactions: prev.transactions.map(t => {
        ...
        // Auto-guess ledger if description changes and current ledger is default/suspense or empty
        if (field === 'description' && typeof value === 'string') {
            ...
            if (!currentLedger || currentLedger === 'Suspense A/c' || currentLedger === 'UPI Suspense') {
                 ...
                 if (guessed !== 'Suspense A/c') {
                     updated.contraLedger = guessed;
                 }
            }
        }
        return updated;
  ...
  const handlePushToTally = async () => {
    setIsPushing(true);
    try {
      // 1. Fetch Existing Ledgers from Tally to avoid duplication or errors
      const existingLedgers = await fetchExistingLedgers();
      
      // 2. Generate XML which includes master creation for missing ledgers
      ...
      if (result.success) {
        ...
        if (onUpdateFile && fileId) {
             onUpdateFile(fileId, { status: 'Success' });
        }
      } else {
        onPushLog('Failed', 'Bank Statement Push Failed', result.message);
        if (onUpdateFile && fileId) {
             onUpdateFile(fileId, { status: 'Failed', error: result.message });
        }
      }
    } catch (e) {
      onPushLog('Failed', 'Network Error', e instanceof Error ? e.message : 'Unknown');
       if (onUpdateFile && fileId) {
             onUpdateFile(fileId, { status: 'Failed', error: 'Network Error' });
        }
    } finally {
      setIsPushing(false);
    }
    ...
    <div className="flex flex-col h-full gap-6 animate-fade-in relative">
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex justify-between items-center">
        <div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <Landmark className="w-6 h-6 text-tally-600" />
        ...
        {step === 2 && (
          <div className="flex items-center gap-3">
             <button 
                ...
                className="flex items-center gap-2 px-3 py-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg text-sm font-medium transition-colors"
             ...
             <button 
               onClick={() => setStep(1)}
               className="text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white text-sm font-medium"
             >
               Upload New
             </button>
             <button 
                ...
                disabled={isPushing}
                className="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2 rounded-lg font-medium flex items-center gap-2 shadow-lg disabled:opacity-70 disabled:cursor-not-allowed"
             >
      ...
      {step === 1 ? (
        <div className="flex-1 flex flex-col items-center justify-center bg-white dark:bg-slate-800 rounded-xl border border-dashed border-slate-300 dark:border-slate-700 p-12">
            ...
            {isProcessing ? (
               ...
               <div className="text-center space-y-4">
                  ...
                  <button 
                    ...
                    className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-lg font-bold shadow-md hover:shadow-lg transition-all"
                  >
                    Select PDF / Image
                  </button>
            ...
            {/* Draft Restore Option */}
            {hasDraft && !isProcessing && (
                <div className="mt-6 flex items-center gap-3 animate-fade-in">
                    <button 
                        onClick={handleRestoreDraft}
                        className="flex items-center gap-2 px-4 py-2 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300 rounded-lg text-sm font-semibold hover:bg-indigo-100 transition-colors"
                    >
        ...
        <div className="flex-1 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 flex flex-col overflow-hidden">
            ...
            <div className="p-4 border-b border-slate-200 dark:border-slate-700 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50 dark:bg-slate-900/50">
               <div className="flex-1">
                  ...
                  <input 
                    ...
                    className="w-full max-w-xs px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm font-semibold"
                     ...
                     {data.transactions.map((txn) => (
                        <tr key={txn.id} className="group hover:bg-slate-50 dark:hover:bg-slate-700/30">
                           ...
                           <td className="p-2">
                              <input 
                                type="text" 
                                value={txn.contraLedger} 
                                onChange={(e) => handleTransactionChange(txn.id, 'contraLedger', e.target.value)}
                                className={`${inputClass} ${(txn.contraLedger === 'Suspense A/c' || txn.contraLedger === 'UPI Suspense') ? 'border-amber-300 dark:border-amber-600 bg-amber-50 dark:bg-amber-900/20' : ''}`}
                                placeholder="Tally Ledger Name"
                              />
                           </td>
                           <td className="p-2 text-center">
                              <button onClick={() => removeTransaction(txn.id)} className="text-slate-400 hover:text-red-500">
                                 <Trash2 className="w-4 h-4" />
                              </button>
                           </td>
                        </tr>
            ...
            <div className="p-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 flex justify-between items-center">
                <button onClick={addTransaction} className="text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:underline">
                   + Add Empty Row
                </button>
