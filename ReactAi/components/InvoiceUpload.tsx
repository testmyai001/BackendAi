...
const InvoiceUpload: React.FC<InvoiceUploadProps> = ({ onFilesSelected, onRestoreDraft }) => {
  ...
  // PAGE-LEVEL AUTO-SCROLL (same as BankStatementManager)
  ...
  return (
    // FULL PAGE SCROLL — NOT INSIDE TABLES
    <div
      ref={pageScrollRef}
      className="
      ...
      <div className="p-8 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center relative z-10">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">
            Upload PDF or Image files to start the OCR extraction process.
          </p>
        </div>
        ...
        <div className="flex items-center gap-1 p-1 bg-slate-100 dark:bg-slate-700 rounded-lg">
          <button
            ...
            className={`
              px-3 py-1.5 text-xs font-bold rounded transition-all flex items-center gap-1
              ${!useMock ? 'bg-white dark:bg-slate-600 shadow text-indigo-700 dark:text-indigo-300'
          ...
          <button
            ...
            className={`
              ...
              ${useMock ? 'bg-white dark:bg-slate-600 shadow text-amber-600 dark:text-amber-400'
                : 'text-slate-400 dark:text-slate-400'}
            `}
      ...
      <div className="p-8 flex-1 flex flex-col relative z-10">
        <div
          className={`
            ...
            ${isDragOver
              ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 scale-[0.98] shadow-lg shadow-indigo-500/20'
              : 'border-slate-300 dark:border-slate-600 hover:border-indigo-400 hover:bg-slate-50 dark:hover:bg-slate-700/50 hover:shadow-md'}
          `}
        ...
        >
          <div className="text-center space-y-6 max-w-lg">
            <div
              className={`
                w-24 h-24 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400
                rounded-3xl flex items-center justify-center mx-auto shadow-sm
                transition-all duration-300
                ${isDragOver ? 'scale-110 shadow-lg shadow-indigo-500/20' : ''}
              `}
            >
              <Upload className={`w-10 h-10 transition-all duration-300 ${isDragOver ? 'animate-bounce' : ''}`} />
            </div>
            ...
            <div className="flex flex-col items-center gap-4">
              <label className="inline-flex items-center justify-center px-8 py-4 bg-gradient-to-r from-indigo-600 to-blue-600 dark:from-indigo-500 dark:to-blue-500 text-white text-sm font-bold rounded-xl hover:from-indigo-700 hover:to-blue-700 dark:hover:from-indigo-600 dark:hover:to-blue-600 cursor-pointer transition-all hover:shadow-xl hover:-translate-y-1 shadow-lg shadow-indigo-600/30">
                BROWSE FILES
